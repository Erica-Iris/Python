...
class Job(object):
    ...
    def __init__(self, script, executor, argv, crontab):
        """Constructor"""
    ...
    def run(self):
        ...
        try:
            cmd = '{} {} {}'.format(self.executor, self.script, self.argv)
...
class Crontab(object):
    """"""
    _jobs = []
    job_config_file = os.path.join(current_path, '../config/jobs.conf')
    def __init__(self):
        """Constructor"""
        self._method = ''
        self._year = None
    ...
    def execute(self, script, executor='python', argv=''):
        ...
        j = Job(script, executor, argv, self)
        self._jobs.append(j)
        self.__init__()
